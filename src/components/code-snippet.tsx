...
export function CodeSnippet({ filePath, code }: CodeSnippetProps) {
  ...
  return (
    <Card className="my-2 bg-muted overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between p-2 border-b bg-muted/50">
        ...
        <Button variant="ghost" size="icon" onClick={handleCopy} className="h-7 w-7 text-muted-foreground hover:bg-accent">
          {copied ? <Check className="h-4 w-4 text-green-500" /> : <Clipboard className="h-4 w-4" />}
        </Button>
      </CardHeader>
      <CardContent className="p-0">
        <ScrollArea className="h-48">
            <pre className="p-3 text-xs whitespace-pre-wrap font-code text-foreground">{code}</pre>
        </ScrollArea>
      </CardContent>
    </Card>
  );
